...
describe('createActionDispatcher', () => {
  it('executes twitch service_call macro steps through the registered service actions', async () => {
    ...
    const macroEngine = new MacroEngine((step, ctx) => executeMacroStep(step, ctx), new InMemoryEventBus())

    const { executeMacroStep: dispatcherExecuteMacroStep } = createActionDispatcher({
      obsService: {
    ...
    executeMacroStep = dispatcherExecuteMacroStep
