...
const cloneRuntimeConfig = (value: RuntimeConfig): RuntimeConfig => ({
  storageKeys: {
    triggers: value.storageKeys.triggers,
    macros: value.storageKeys.macros,
  },
  twitch: value.twitch ? { ...value.twitch } : undefined,
  obs: value.obs ? { ...value.obs } : undefined,
})
...
export const loadRuntimeConfig = async (
  ...
  if (!storage) {
    return cloneRuntimeConfig(fallback)
  }

  const raw = await storage.load<unknown>(RUNTIME_CONFIG_STORAGE_KEY)
  return isRuntimeConfig(raw) ? cloneRuntimeConfig(raw) : cloneRuntimeConfig(fallback)
}

export const persistRuntimeConfig = async (
  storage: StoragePort | undefined,
  ...
  await storage.save(RUNTIME_CONFIG_STORAGE_KEY, runtimeConfig)
}
