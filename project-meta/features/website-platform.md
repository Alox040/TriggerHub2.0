...
- Das Routing modelliert `private_prelaunch`, `invite_only` und `public_product` ueber `modeOverrides`.
- Die Runtime-Konfiguration in `website/src/config/runtimeConfig.ts` akzeptiert aktuell effektiv nur `private_prelaunch`.
- Das Profilsystem liest und schreibt ueber `/api/profile/me` mit serverseitiger Session- und CSRF-Anbindung; lokaler Speicher ist nur noch Cache/Fallback.
- Der Website-Build ist erfolgreich; vor dem Build laeuft ein Prelaunch-Sicherheitscheckskript.
...
- Das Prebuild-Skript verhindert unsichere Prelaunch-Builds.
- Die Website hat einen eigenen Buildpfad und ist architektonisch klar vom Desktop getrennt.
...
- `runtimeConfig.ts` auf den bereits im Route-Manifest modellierten Modusumfang bringen, sobald der Sicherheitsrahmen dafuer belegt ist.
